<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="区域名称" prop="id" sortable="custom" align="center" width="400">
        <template slot-scope="{row}">
          <span>{{ row.areaName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="街道名称名称" prop="streetName" sortable="custom" align="center" width="400">
        <template slot-scope="{row}">
          <span>{{ row.streetName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="小区名称" prop="communityName" sortable="custom" align="center" width="400">
        <template slot-scope="{row}">
          <span>{{ row.communityName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="最后操作人" prop="id" sortable="custom" align="center" width="400">
        <template slot-scope="{row}">
          <span>{{ row.updateUser }}</span>
        </template>
      </el-table-column>
      <el-table-column label="最后操作时间" prop="id" sortable="custom" align="center" width="400">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        <el-form-item label="区域名称" prop="areaName">
          <el-select v-model="temp.areaName" class="filter-item" placeholder="请选择">
        ...
        <el-form-item label="街道名称" prop="streetName">
          <el-select v-model="temp.streetName" class="filter-item" placeholder="请选择">
